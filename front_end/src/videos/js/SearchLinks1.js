...
function modifyURL(append){
  if(document.getElementById("PageResults").classList.contains("showMainPlayer")){
    var urlcheck = document.getElementsByClassName('mainplayer')[0].children[0].children[0].children[0].children[0].children[0].getAttribute('urlcheck');
    window.history.replaceState('data', '234', 'key?search='+ append +'&url='+ urlcheck);
  }else{
...
function resetURL(){
  ...
  if(document.getElementById("PageResults").classList.contains("showMainPlayer")){
    var urlKey = document.getElementsByClassName('mainplayer')[0].children[0].children[0].children[0].children[0].children[0].getAttribute('urlcheck');
    window.history.replaceState('data', '234', 'key?url='+ urlKey);
  }else if(document.getElementById("Search").value != ""){
    var searchKey = document.getElementById("Search").value;
    window.history.replaceState('data', '234', 'key?search='+ searchKey);
  }
}
...
function Search_Commands(SearchString, display){
  ...
  if(array[0] == "g"){
    ...
    document.getElementById("PageResults").innerHTML = null;
...
function SetScroll(){
  if(document.querySelector(".AfterEnter").scrollTop > 0){
    ...
    document.querySelector(".AE2").style.boxShadow = "inset 0 7px 9px -8px rgba(190, 43, 43, 1)";
    let z = 1;
